...
void Chess_Board_Init(void){
  ...
  IOMUX->SECCFG.PINCM[PB15INDEX] = 0x00040081; //Button 1
  IOMUX->SECCFG.PINCM[PB16INDEX] = 0x00040081; //Button 2
  IOMUX->SECCFG.PINCM[PB17INDEX] = 0x00040081; //Button 3
...
void getMove(char *move) {
    ...
    uint8_t step = 0;  // 0 = selecting source, 1 = selecting destination

    while (step < 2) {
        uint8_t col_index = 0, row_index = 0;

        // Select both column and row in a single loop
        while (!(GPIOB->DIN31_0 & 0x20000)) {

            if (GPIOB->DIN31_0 & 0x8000) {
                col_index = (col_index + 1) & 8;  // Cycle through 'a' to 'h'
                printf("DEBUG: Button 1 pressed, Column Selected: %c (index %d)\n", columns[col_index], col_index);
                Clock_Delay(800000); // 10ms, to debounce switch
            }
            if (GPIOB->DIN31_0 & 0x10000) {
                row_index = (row_index + 1) & 8;  // Cycle through '1' to '8'
                printf("DEBUG: Button 2 pressed, Row Selected: %c (index %d)\n", rows[row_index], row_index);
                Clock_Delay(800000); // 10ms, to debounce switch
            }
        }

        move[step * 2] = columns[col_index];    // Store column letter
        move[step * 2 + 1] = rows[row_index];   // Store row number

    

        while ((GPIOB->DIN31_0 & 0x20000));  // Wait for button release

        step++;  // Move to next step (destination)
    }

    move[4] = '\0';  // Null-terminate

    printf("Full move stored: %s\n", move);
}
