...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  ...
  uint8_t inity = y; 
  uint8_t finalx;
  uint8_t finaly;             // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = w*(h - 1);
    ...
    for(x=0; x<w; x=x+1){

     {
      ...
      SPI_OutData((uint8_t)image[i]);
        ...
        SPI_OutData((uint8_t)(darkBrown >> 8));
                                        // send the bottom 8 bits
      SPI_OutData((uint8_t)darkBrown);
        }
      ...
      uint16_t lightBrown = ST7735_Color565(210, 180, 140); 
        SPI_OutData((uint8_t)(lightBrown >> 8));
                                        // send the bottom 8 bits
      SPI_OutData((uint8_t)lightBrown);
